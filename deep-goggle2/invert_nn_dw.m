...
if ~isempty(opts.net2)
    pre_fmask=opts.net2.fmask;
    [opts.net2,opts2] = invert_nn_pre(opts.net2, opts.net2.mask, varargin);
    opts.net2.fmask = pre_fmask;
end
...
switch opts.task
case 0
y0_sigma = norm(squeeze(y0(find(net.layers{end}.mask(:))))) ;
case {1,2}
y0_sigma = 1;
end
...
if opts.do_dc==2
    PYR_FILTER = [1;3;3;1]/sqrt(8);
    PYR_EDGES = 'repeat';
    Zfilt = PYR_FILTER(:);
    edges = PYR_EDGES;
    MAX_PYR_DEPTH = 2;
    S = min(MAX_PYR_DEPTH, floor(log2(min(227))));
    [pyramid_Zmeta] = buildGpyr_matrix_meta([227 227], S, Zfilt, edges);

    %Zpyr = zeros(size(pyramid_Zmeta.Ac,1),3);
    Zpyr = 100*randn(size(pyramid_Zmeta.Ac,1),3);
    %keyboard
    %for i=1:3;Zpyr(:,i) = buildGpyr_matrix(double(x(:,:,i)), pyramid_Zmeta);end
...
if ~isempty(opts.m67)
    ...
    if ~isempty(opts.net2) && isfield(opts.net2,'m67')
        res2 = vl_simplenn_dw(opts.net2, x); % x is the random initialized image
        res2(19).aux(:)=0;
        res2(19).aux(:,:,find(opts.net2.m67{1}))=2;
        res2(22).aux(:)=0;
        res2(22).aux(:,:,find(opts.net2.m67{2}))=2;
    end
...
if ~isempty(opts.net2) 
    %keyboard
  res2 = vl_simplenn_dw(opts.net2, x, single(1),res2) ;
  end

 %save db_grad res;return
  if ~isempty(opts.regu)
  ...
  if ~isempty(opts.np) &&opts.np.lambdaP(t) > 0 && mod(t,opts.np.step)==0 % Cost and derivative for TV\beta norm
      tmp_x=x;
      if ~isempty(opts.im_w)
        tmp_x=reshape(reshape(x,[],3)*opts.im_w',size(x));
      end
    [r_,dr_,dsp_im] = patch_np(tmp_x,opts.np,opts.xmask,opts.task,t) ;
    E(4,t) = opts.np.lambdaP(t) * r_ ;
    %imagesc([dr(:,:,1) dr_(:,:,1)*opts.np.lambdaP]),colorbar
    %keyboard
    dr = dr + opts.np.lambdaP(t) * dr_ ;
  else
    E(4,t) = 0;
  end
  %{
%}
  ...
  if ~isempty(opts.net2)
      E(1,t) =  E(1,t) + opts.net2.lambdaD*res2(end).x/(y0_sigma^2);
  end


  E(2:4,t) = E(2:4,t) / (x0_sigma^2) ;
  E(5,t) = sum(E(1:4,t)) ;
  ...
  if ~isempty(opts.net2)
      dr_d = dr_d + opts.net2.lambdaD*res2(1).dzdx;
    %imagesc([res(1).dzdx(:,:,1) res2(1).dzdx(:,:,1) dr_d(:,:,1)/opts.net2.lambdaD])
  end
  %keyboard
  ...
  x_momentum = opts.momentum * x_momentum ...
    ...
    if opts.dsp>=0
  fprintf('iter:%05d sq. max_grad: %3.4g, %3.4g, %3.4g  err:%5.4g, %5.4g, %5.4g; %5.4g; obj:%5.4g;\n', t, tmp_max,dsp_p*mean(abs(dr_d(:))),mean(abs(dr_d(:))),E(1,end),E(2,end),E(3,end), E(4,end), E(5,end)) ;
  end
  ...
  if opts.do_thres
      tmp_m = x+net.normalization.averageImage; 
      tmp_m(tmp_m<0)=0;
      tmp_m(tmp_m>255)=255;
      x = tmp_m-net.normalization.averageImage; 
  end
  ...
  if opts.dsp>0 && mod(t-1,opts.dsp)==0
    ...
    case 1
        ...
        subplot(3,2,[2 4]) ;
        imagesc(uint8(opts.denormalize(dsp_im)));
    axis image ; 
        end
    end
