function res = vl_simplenn_dw(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'relu-s'
      res(i+1).x = 0.5*(1/sqrt(pi*opts.c)*exp(-opts.c*res(i).x.^2)+x.*(1+erf(sqrt(opts.c).*res(i).x)));
    case 'noffset'
...
if doder
  ...
  for i=n:-1:1
    ...
    switch l.type
      ...
      case 'relu-s'
        res(i).dzdx = 0.5*(1+erf(sqrt(opts.c)*res(i).x)).* res(i+1).dzdx ;
      case 'relu'
    ...
    res(i).backwardTime = toc(res(i).backwardTime) ;
      ...
      % dw: extra masking
      if ~isempty(mask_id) &&ismember(i,mask_id)
        rid = find(mask_id==i);
        tmp_mask = net.fmask{rid}{2};
        res(i).dzdx = bsxfun(@times,res(i).dzdx,tmp_mask);
      end

      % dw: extra regularization
      if ~isempty(regu_id) &&ismember(i,regu_id)
        rid = find(regu_id==i);
        regu_m = reshape(net.regu{rid}{2},1,1,[]);
        % if pool layer, don't change 0-mask, still not work
        res(i).dzdx = res(i).dzdx+ 2*net.regu{rid}{3}*(res(i).x>0).*bsxfun(@minus,res(i).x,regu_m);
      end
  end
end
